...
function WeeklyReport() {
  ...
  const showSaturday = filteredReports.some(r => (r.saturdayMinutes ?? 0) > 0);
  const dayFields = showSaturday ? allDays : allDays.slice(0, 5);
  ...
  const columns: GridColDef[] = [
    ...
    ...dayFields.map(({ field, label }, i) => ({
      field,
      headerName: startDate ? `${label} (${getDayDate(i)})` : label,
      flex: 1,
    })),
    { field: "totalWorkingHours", headerName: "Total Hours",  flex: 1 },
      ...
      <div className="card border-0 shadow-sm mb-4" style={{ borderRadius: "12px" }}>
        <div className="card-body p-4">
          <div className="row g-3 align-items-end">
            ...
            <div className="col-md-2">
              <button className="btn btn-success w-100" onClick={() => reportService.exportWeekly(startDate, endDate)} disabled={!startDate || !endDate}>
                📥 Export Excel
              </button>
            </div>
          </div>
      ...
      {filteredReports.length > 0 && (
        <div className="row g-3 mb-4">
          {[
            ...
            { title: "Weekly Records",   value: filteredReports.length, icon: "📊", color: "#2ec4b6" },
